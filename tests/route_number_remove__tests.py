...
class RouteNumberRemoveTests(NumberServiceInitializedTestCase):

    def test_route_remove_number__with_admin_auth_token(self):
        # Given
        ...
        # When
        response = self.app.post("/number/remove",
    ...
    def test_route_remove_number__with_user_auth_token(self):
        ...
        number_id = "com.myapp"
        with postgres.get_db_conn() as conn:
            user.add(conn, "admin", "oranges", auth_token=user_auth_token, is_admin=False)
            number.add(conn, number_id)
        # When
        response = self.app.post("/number/remove",
                                 data={"number_id": number_id},
    ...
    def test_route_remove_number__with_invalid_number(self):
        # Given
        admin_auth_token = "abc123"
        number_id = "com.myapp"
        with postgres.get_db_conn() as conn:
