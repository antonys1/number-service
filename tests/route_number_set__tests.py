...
class RouteNumberSetTests(NumberServiceInitializedTestCase):
    ...
    def test_route_number_set__with_valid_auth_token(self):
        ...
        with postgres.get_db_conn() as conn:
            user.add(conn, user_id, "oranges", auth_token=admin_auth_token, is_admin=True)
            number.add(conn, number_id)
            number.add_user(conn, number_id, user_id)
        # When
        response = self.app.post("/number/set",
                                 data={"number_id": number_id, "new_number": 2},
    ...
    def test_route_number_set__with_invalid_number(self):
        ...
        response = self.app.post("/number/set",
                                 data={"number_id": invalid_number_id, "new_number": 2},
                                 headers=self.authorization_custom(admin_auth_token))
        # Then
